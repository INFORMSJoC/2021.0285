...
def solve_with_sqp(nfe_x, nfe_t):
...
def solve_with_ipopt(nfe_x, nfe_t):
    m = build_burgers_model(nfe_x=nfe_x, nfe_t=nfe_t)
    opt = pe.SolverFactory('ipopt')

    # These options make ipopt faster
    # These features are not needed for this problem
    opt.options['constr_mult_init_max'] = 0.0
    opt.options['linear_system_scaling'] = 'none'
    opt.options['linear_scaling_on_demand'] = 'no'
    opt.options['nlp_scaling_method'] = 'none'
    opt.options['min_refinement_steps'] = 0
    opt.options['max_refinement_steps'] = 0
    opt.options['max_soc'] = 0
    opt.options['ma27_pivtol'] = 1e-6
    opt.options['ma27_pivtolmax'] = 1e-6
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nfe_x', type=int, required=True, help='number of finite elements in space')
    parser.add_argument('--nfe_t', type=int, required=True, help='number of finite elements in time')
    args = parser.parse_args()

    print('Solving with SQP')
    solve_with_sqp(args.nfe_x, args.nfe_t)
    print('\nSolving with Ipopt')
    solve_with_ipopt(args.nfe_x, args.nfe_t)
